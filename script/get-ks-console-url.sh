...
"LoadBalancer")
    KS_CONSOLE_SVC_EIP=$(echo ${KS_CONSOLE_SVC_CONTENT} | jq '.status.loadBalancer.ingress[0].ip'| sed 's/\"//g')
    if [ "${KS_CONSOLE_SVC_EIP}" == "null" ]
    then
        echo "Updating EIP address"
        exit
    else
        echo http://${KS_CONSOLE_SVC_EIP}:30880
        exit
    fi
    ;;
"NodePort")
